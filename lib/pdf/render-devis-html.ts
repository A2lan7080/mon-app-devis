...
export type DevisPdfData = Devis & {
...
function getStatutPdfClasse(statut: DevisPdfData["statut"]) {
  switch (statut) {
    case "Accepté":
...
function getPreuveElectroniqueHtml(devis: DevisPdfData) {
...
function getSignatureClientHtml(devis: DevisPdfData) {
...
type RenderDevisHtmlOptions = {
  autoPrint?: boolean;
};

export function renderDevisHtml(
  devisSelectionne: DevisPdfData,
  entreprise: EntrepriseSettings,
  options: RenderDevisHtmlOptions = {}
) {
  ...
  return `
    ...
    <html lang="fr">
      ...
      <body>
        ...
        ${options.autoPrint ? getPrintScript() : ""}
      </body>
    </html>
  `;
}
